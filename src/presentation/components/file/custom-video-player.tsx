...
import { Box, Button, IconButton, Menu, MenuItem, Slider, Stack, Typography, useTheme } from '@mui/material';
  ...
  return (
    ...
    >
      ...
      {showControls && (
        ...
        >
          <IconButton onClick={togglePlay} sx={{ color: theme.palette.primary.main, fontSize: 24, p: 1.5 }}>
            {isPlaying ? <PauseIcon fontSize="inherit" /> : <PlayArrowIcon fontSize="inherit" />}
          </IconButton>

          <IconButton onClick={toggleMute} sx={{ color: theme.palette.primary.main, fontSize: 24, p: 1.5 }}>
          ...
          <Slider
            ...
            max={1}
            step={0.01}
            sx={{ width: 80, color: theme.palette.primary.main, height: 2 }}
          />
          <Box flexGrow={2} />
