...
// export function CustomVideoPlayer({ src, fullscreen }: CustomVideoPlayerProps) {
...
//   const handleProgressChange = (_event: Event | SyntheticEvent, value: number | number[]) => {
//     if (!videoRef.current || typeof value !== 'number') return;
//     videoRef.current.currentTime = (value / 100) * duration;
//     setProgress(value);
//   };

//   const handleVolumeChange = (_event: Event | SyntheticEvent, value: number | number[]) => {
//     if (!videoRef.current || typeof value !== 'number') return;
...
export function CustomVideoPlayer({ src, fullscreen }: CustomVideoPlayerProps) {
  ...
  const hideControlsTimer = useRef<number | ReturnType<typeof setTimeout> | null>(null);
  ...
  const togglePlay = () => {
    ...
    if (video.paused) {
      void video.play(); // Fix 2: Add void to handle floating promise
      setIsPlaying(true);
    } else {
      video.pause();
      setIsPlaying(false);
  ...
  return (
    ...
    >
      {/* eslint-disable-next-line jsx-a11y/media-has-caption -- controlled player with custom UI */}
      <video
        ref={videoRef}
        src={src}
        controls={false}
