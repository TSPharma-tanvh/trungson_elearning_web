...
interface LinearCourseDetailProps {
  open: boolean;
  courseId: string | null;
  onClose: () => void;
}

function LinearCourseDetails({ course, fullScreen }: { course: CourseDetailResponse; fullScreen: boolean }) {
...
export default function CourseDetailForm({ open, courseId, onClose }: LinearCourseDetailProps) {
  const { t } = useTranslation();
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg" fullScreen={fullScreen}>
      ...
      <DialogContent dividers sx={{ p: 0 }}>
        {loading || !course ? (
          ...
          <LinearCourseDetails course={course} fullScreen={fullScreen} />
        )}
      </DialogContent>
    </Dialog>
  );
