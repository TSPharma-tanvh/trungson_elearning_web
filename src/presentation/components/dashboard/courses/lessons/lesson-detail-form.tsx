...
function LessonDetails({ lesson, fullScreen }: { lesson: LessonDetailResponse; fullScreen: boolean }) {
  ...
  const renderCourse = () => {
    if (!lesson.collection?.course) return null;

    const course = lesson.collection?.course;
      ...
      <Card sx={{ mb: 2 }}>
        ...
        <CardContent>
          <Grid container spacing={2}>
            ...
            {renderField('collectionName', lesson.collection?.course?.name)}
          </Grid>
        </CardContent>
      </Card>
    );
        ...
        <Card sx={{ mb: 2 }}>
          ...
          <CardContent>
            <Grid container spacing={2}>
              ...
              {renderField('lessonDetail', lesson.detail)}
              {renderField('courseName', lesson.collection?.course?.name)}
              {renderField('collectionName', lesson.collection?.course?.name)}
