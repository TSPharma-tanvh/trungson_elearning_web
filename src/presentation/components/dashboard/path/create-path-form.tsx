...
export function CreateCoursePathDialog({ disabled = false, onSubmit, loading = false, open, onClose }: Props) {
  ...
  useEffect(() => {
    const updateRows = () => {
      ...
      // Base height for other elements (header, other fields, button, padding)
      let otherElementsHeight = 300; // Default for standard screens
      if (windowHeight < 600) {
        otherElementsHeight = 250; // Smaller screens
      } else if (windowHeight > 1000) {
        otherElementsHeight = 350; // Larger screens
      }

      // Adjust row height based on aspect ratio
      const rowHeight = aspectRatio > 1.5 ? 22 : 24; // Slightly smaller rows for wide screens
