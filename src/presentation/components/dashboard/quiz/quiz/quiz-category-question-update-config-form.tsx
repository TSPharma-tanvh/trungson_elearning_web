...
import { QuizCategoryConfigUpdate } from '@/domain/models/quiz/request/update-quiz-request';
...
interface QuizCategoryConfigUpdateFormProps {
  value: QuizCategoryConfigUpdate[];
  onChange: (value: QuizCategoryConfigUpdate[]) => void;
  categoryUsecase: CategoryUsecase | null;
}

export function QuizCategoryConfigUpdateForm({ value, onChange, categoryUsecase }: QuizCategoryConfigUpdateFormProps) {
  const { t } = useTranslation();

  const updateItems = (next: QuizCategoryConfigUpdate[]) => {
    onChange(next.map((i, idx) => new QuizCategoryConfigUpdate({ ...i, order: idx + 1 })));
  };
  ...
  const updateField = <K extends keyof QuizCategoryConfigUpdate>(
    order: number,
    field: K,
    fieldValue: QuizCategoryConfigUpdate[K]
  ) => {
    updateItems(
      value.map((i) => (i.order === order ? new QuizCategoryConfigUpdate({ ...i, [field]: fieldValue }) : i))
    );
  };
