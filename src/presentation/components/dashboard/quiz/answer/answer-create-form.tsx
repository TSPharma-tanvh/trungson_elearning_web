...
export function CreateAnswerForm({ open, onClose, onSubmit }: EditAnswerDialogProps) {
  ...
  const [formData, setFormData] = useState<CreateAnswerRequest>(new CreateAnswerRequest({}));
  ...
  useEffect(() => {
    if (open) {
      const newFormData = new CreateAnswerRequest({
  ...
  const handleChange = <K extends keyof CreateAnswerRequest>(field: K, value: CreateAnswerRequest[K]) => {
    setFormData((prev) => new CreateAnswerRequest({ ...prev, [field]: value }));
  };
