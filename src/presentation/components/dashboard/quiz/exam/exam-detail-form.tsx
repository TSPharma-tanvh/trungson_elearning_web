...
function ExamDetails({ quiz, fullScreen }: { quiz: QuizResponse; fullScreen: boolean }) {
      ...
      <Card sx={{ mb: 2 }}>
        ...
        <CardContent>
          <Grid container spacing={2}>
            ...
            {renderField(
              'startTime',
              quiz.startDate ? DateTimeUtils.formatDateTimeToDateString(quiz.startDate) : undefined
            )}
            {renderField('endTime', quiz.endDate ? DateTimeUtils.formatDateTimeToDateString(quiz.endDate) : undefined)}
            ...
            {renderField('duration', quiz.isFixedQuiz ? t('duration') : t('time'))}
...
export default function QuizDetailForm({ open, quizId, onClose }: ExamDetailFormProps) {
  const { t } = useTranslation();
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg" fullScreen={fullScreen}>
      ...
      <DialogContent dividers sx={{ p: 0 }}>
        {loading || !course ? (
          ...
          <ExamDetails quiz={course} fullScreen={fullScreen} />
        )}
      </DialogContent>
    </Dialog>
  );
