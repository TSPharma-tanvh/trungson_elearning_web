...
export class UserPathProgressRepoImpl implements UserPathProgressRepository {
  ...
  async getUserPathProgressById(id: string): Promise<ApiResponse> {
    try {
      const response = await customApiClient.get<ApiResponse>(apiEndpoints.userPathProgress.getById(id));
      const apiResponse = response.data;
  ...
  async deleteUserPathProgress(id: string): Promise<ApiResponse> {
    try {
      const response = await customApiClient.delete<ApiResponse>(apiEndpoints.userPathProgress.delete(id));
